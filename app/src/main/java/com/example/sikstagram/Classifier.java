...
public class Classifier {
    ...
    public String[] predict(Bitmap bitmap, String result_type) {
        ...
        // 입력한 이미지를 Tensor 형태로 변환
        Tensor tensor = preprocess(croppedBitmap, 224);

        // 이미지 텐서를 인풋으로 모델 포워딩, 아웃풋을 받음
        IValue inputs = IValue.from(tensor);
        Tensor outputs = model.forward(inputs).toTensor();

        // 아웃풋을 Float 배열로 받아, argMax시행으로 Top5 도출
        float[] scores = outputs.getDataAsFloatArray();
        int[] classIndexes = argMax(scores);

        // result_type 입력에 따라 각각 영문, 한글, 레이블 인덱스로 리턴
        if (result_type == "en") {
